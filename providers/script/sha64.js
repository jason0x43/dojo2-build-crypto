(function (deps, factory) {
     ...
     */
    function rotateRight(dst, src, shift) {
     ...
     */
    function reverseRotateRight(dst, src, shift) {
     ...
     */
    function shiftRight(dst, src, shift) {
        dst[0] = (src[0] >>> shift) | (src[1] << (32 - shift));
        dst[1] = (src[1] >>> shift);
    }
    /**
    ...
    function add(dst, x, y) {
        var w0 = (x[0] & 0xFFFF) + (y[0] & 0xFFFF);
        var w1 = (x[0] >>> 16) + (y[0] >>> 16) + (w0 >>> 16);
        var w2 = (x[1] & 0xFFFF) + (y[1] & 0xFFFF) + (w1 >>> 16);
        var w3 = (x[1] >>> 16) + (y[1] >>> 16) + (w2 >>> 16);
    ...
    function add4(dst, a, b, c, d) {
        var w0 = (a[0] & 0xFFFF) + (b[0] & 0xFFFF) + (c[0] & 0xFFFF) + (d[0] & 0xFFFF);
        var w1 = (a[0] >>> 16) + (b[0] >>> 16) + (c[0] >>> 16) + (d[0] >>> 16) + (w0 >>> 16);
        var w2 = (a[1] & 0xFFFF) + (b[1] & 0xFFFF) + (c[1] & 0xFFFF) + (d[1] & 0xFFFF) + (w1 >>> 16);
        var w3 = (a[1] >>> 16) + (b[1] >>> 16) + (c[1] >>> 16) + (d[1] >>> 16) + (w2 >>> 16);
        dst[0] = (w0 & 0xFFFF) | (w1 << 16);
        dst[1] = (w2 & 0xFFFF) | (w3 << 16);
    }
    /**
    ...
    function add5(dst, a, b, c, d, e) {
        var w0 = (a[0] & 0xFFFF) + (b[0] & 0xFFFF) + (c[0] & 0xFFFF) + (d[0] & 0xFFFF) + (e[0] & 0xFFFF);
        var w1 = (a[0] >>> 16) + (b[0] >>> 16) + (c[0] >>> 16) + (d[0] >>> 16) + (e[0] >>> 16) + (w0 >>> 16);
        var w2 = (a[1] & 0xFFFF) + (b[1] & 0xFFFF) + (c[1] & 0xFFFF) + (d[1] & 0xFFFF) + (e[1] & 0xFFFF) + (w1 >>> 16);
        var w3 = (a[1] >>> 16) + (b[1] >>> 16) + (c[1] >>> 16) + (d[1] >>> 16) + (e[1] >>> 16) + (w2 >>> 16);
        dst[0] = (w0 & 0xFFFF) | (w1 << 16);
        dst[1] = (w2 & 0xFFFF) | (w3 << 16);
    }
    // constant K array
     ...
     */
    function sha64(bytes, _hash) {
        var numBits = bytes.length * 8;
        var words = base_1.bytesToWords(bytes);
        // Initialize the hash
        var hash = [];
        for (var i = 0, count_1 = _hash.length; i < count_1; i += 2) {
        ...
        return base_1.wordsToBytes(ret);
    }
    ;
    var HASH_384 = [
...
//# sourceMappingURL=../../_debug/providers/script/sha64.js.map